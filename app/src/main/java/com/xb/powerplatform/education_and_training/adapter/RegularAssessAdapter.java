...
import com.xb.powerplatform.education_and_training.activity.AssessResultActivity;
import com.xb.powerplatform.education_and_training.activity.RegularAssessActivity;
...
public class RegularAssessAdapter extends PagerAdapter {

    RegularAssessActivity mContext;
    // 传递过来的页面view的集合
    List<View> viewItems;
    // 每个item的页面view
    View convertView;
    // 传递过来的所有数据
    List<RegularAssess.BodyBean.NewListBean> dataItems;
    //答案集合
    ...
    public RegularAssessAdapter(RegularAssessActivity context, List<View> viewItems, List<RegularAssess.BodyBean.NewListBean> beanList, RegularAssess.BodyBean.RuleBean beanRule) {
        mContext = context;
        ...
        if (beanList.size() < mPosition) {
            ...
            if (answer != null) {
                if (!answer.equals(dataItems.get(mPosition1).getQuAnswer())) {
                    ...
                    if (dataItems.get(mPosition1).getQuType().equals("0")) {
                        if (dataItems.get(mPosition1).getQuCategory().equals("0")) {
                            //单选安全知识
                            code = code + beanRule.erScoreRadioSafety;
                        } else if (dataItems.get(mPosition1).getQuCategory().equals("1")) {
                            //单选法律法规
                            code = code + beanRule.erScoreRadioLaws;
                        } else if (dataItems.get(mPosition1).getQuCategory().equals("2")) {
                            //单选专业知识
                            code = code + beanRule.erScoreRadioMajor;
                        }
                    } else if (dataItems.get(mPosition1).getQuType().equals("1")) {
                        if (dataItems.get(mPosition1).getQuCategory().equals("0")) {
                            //多选安全知识
                            code = code + beanRule.erScoreMultiSafety;
                        } else if (dataItems.get(mPosition1).getQuCategory().equals("1")) {
                            //多选法律法规
                            code = code + beanRule.erScoreMultiLaws;
                        } else if (dataItems.get(mPosition1).getQuCategory().equals("2")) {
                            //多选专业知识
                            code = code + beanRule.erScoreMultiMajor;
                        }
                    } else if (dataItems.get(mPosition1).getQuType().equals("2")) {
                        if (dataItems.get(mPosition1).getQuCategory().equals("0")) {
                            //判断安全知识
                            code = code + beanRule.erScoreJudgeSafety;
                        } else if (dataItems.get(mPosition1).getQuCategory().equals("1")) {
                            //判断法律法规
                            code = code + beanRule.erScoreJudgeLaws;
                        } else if (dataItems.get(mPosition1).getQuCategory().equals("2")) {
                            //判断专业知识
                            code = code + beanRule.erScoreJudgeMajor;
                        }
                    }
                }
                beanList.add(answer);
    ...
    public void upData() {
        ...
        Intent intent = new Intent(mContext, AssessResultActivity.class);
